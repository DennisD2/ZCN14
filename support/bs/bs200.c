...
/* having a beep doesn't match well with NC100/NC150 */
/* (the void casts make it awkward to just remove though) */
#define	beep()	sleep(0)
...
#define COLWIDTH	80
#define PROMPTLINE	18			/* prompt line */
...
static void initgame()
{
    ...
    /* compressing this stuff is what fits it in 21 lines -rjm */
    (void) move(HYBASE,  HXBASE);printf(
		    "Ship positioning: move cursor, type ship initial,");
    (void) move(HYBASE+1,HXBASE);printf( 
		    "type direction (or `r' for random dir). Or, type");
    (void) move(HYBASE+2,HXBASE);printf(
		    "`R' to place all remaining ships randomly.");
    (void) move(MYBASE,   MXBASE);printf("Aiming keys:");
    (void) move(SYBASE,   SXBASE);printf("y k u    7 8 9");
    (void) move(SYBASE+1, SXBASE);printf("h-+-l    4-+-6");
    (void) move(SYBASE+2, SXBASE);printf("b j n    1 2 3");
